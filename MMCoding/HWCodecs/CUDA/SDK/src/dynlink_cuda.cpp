...
static LPCWSTR RtcLibName = L"nvrtc64_112_0.dll";
#else
static LPCSTR __CudaLibName = "nvcuda.dll";
static LPCSTR RtcLibName = "nvrtc64_112_0.dll";
#endif

typedef HMODULE CUDADRIVER;

static CUresult LOAD_LIBRARY(const wchar_t* name, CUDADRIVER *pInstance)
{
    *pInstance = LoadLibrary(name);
...
#define GET_PROC_EX(name, alias, required)                     \
    ...
    /*if (alias == NULL && required) {                                    \
        std::cerr << "Failed to find required function " << #name << " in " << __CudaLibName << "\n";   \
        }*/

#define GET_PROC_EX_V2(name, alias, required)                           \
    alias = (t##name *)GetProcAddress(CudaDrvLib, STRINGIFY(name##_v2));\
    /*if (alias == NULL && required) {                                    \
        std::cerr << "Failed to find required function " << STRINGIFY(name##_v2) << " in " << __CudaLibName << "\n";   \
        }*/
...
static char RtcLibName[] = "libnvrtc.so";
#endif

typedef void *CUDADRIVER;

static CUresult LOAD_LIBRARY(const char * name, CUDADRIVER *pInstance)
{
    *pInstance = dlopen(name, RTLD_NOW);
...
#if INIT_CUDA_RTC
inline CUresult CUDAAPI cuInitRTC(unsigned int Flags, int cudaVersion)
{
    CUDADRIVER CudaDrvLib;
    if (LOAD_LIBRARY(RtcLibName, &CudaDrvLib) != CUDA_SUCCESS || !CudaDrvLib)
    {
        //printf("LoadLibrary \"%s\" failed!\n", RtcLibName);
        return CUDA_ERROR_NOT_INITIALIZED;
    }

    GET_PROC(nvrtcCreateProgram);
    GET_PROC(nvrtcDestroyProgram);
    GET_PROC(nvrtcCompileProgram);
    GET_PROC(nvrtcGetPTXSize);
    GET_PROC(nvrtcGetPTX);
    GET_PROC(nvrtcGetProgramLogSize);
    GET_PROC(nvrtcGetProgramLog);

    IsRtcLoaded = nvrtcCreateProgram;

    return IsRtcLoaded ? CUDA_SUCCESS : CUDA_ERROR_NOT_INITIALIZED;
}
#endif



CUresult CUDAAPI cuInit(unsigned int Flags, int cudaVersion, int& foundCudaVersion)
{
    CUDADRIVER CudaDrvLib;
    int driverVer = 1000;

    CHECKED_CALL(LOAD_LIBRARY(__CudaLibName, &CudaDrvLib));
    ...
    foundCudaVersion = driverVer;
    if (driverVer < cudaVersion)
    {
    ...
    {
        GET_PROC(cuDeviceGetUuid);
        GET_PROC(cuDeviceGetLuid);
    }
    ...
    if (cudaVersion >= 4000)
    {
        ...
        GET_PROC_V2(cuDevicePrimaryCtxRelease);
        GET_PROC_V2(cuDevicePrimaryCtxReset);
        GET_PROC_V2(cuDevicePrimaryCtxSetFlags);
        ...
        GET_PROC(cuTexObjectGetResourceDesc);
        GET_PROC(cuTexObjectGetTextureDesc);
        GET_PROC(cuTexObjectGetResourceViewDesc);

        GET_PROC(cuMemcpyAsync);
        GET_PROC(cuMemcpyPeerAsync);
        GET_PROC(cuDeviceCanAccessPeer);
        GET_PROC(cuCtxEnablePeerAccess);
        GET_PROC(cuCtxDisablePeerAccess);
    }
    ...
    if (driverVer >= 4000)
    {
        ...
        GET_PROC(cuMemAddressReserve);
        GET_PROC(cuMemAddressFree);
        GET_PROC(cuMemCreate);
        GET_PROC(cuMemRelease);
        GET_PROC(cuMemMap);
        GET_PROC(cuMemUnmap);
        GET_PROC(cuMemSetAccess);
        GET_PROC(cuMemGetAccess);
        GET_PROC(cuMemExportToShareableHandle);
        GET_PROC(cuMemImportFromShareableHandle);
        GET_PROC(cuMemGetAllocationGranularity);
        GET_PROC(cuMemGetAllocationPropertiesFromHandle);
        GET_PROC(cuMemRetainAllocationHandle);
    }
...
#if INIT_CUDA_RTC
    cuInitRTC(0, __CUDA_API_VERSION);
#endif
